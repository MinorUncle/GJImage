...
static GLubyte* metalLayerBuffer;

@interface CAMetalLayer(Swizzling)
@end
static uint8_t* layerBuffer;
...
- (nullable id <CAMetalDrawable>)nextDrawable_swizzling{
    ...
    if (drawable.texture == nil) {
        ...
        if (@available(iOS 11.0, *)) {
            @synchronized (self) {
                ...
                size_t w = IOSurfaceGetWidth(surface)*4;
                size_t row = IOSurfaceGetBytesPerRowOfPlane(surface, 0);
                if (layerBuffer == nil) {
                    layerBuffer = malloc(w*h*4);
                }
                for (int i = 0; i<h; i++) {
                    memcpy(layerBuffer+i*w, baseAddr+i*row, w);
                }
                NSLog(@"texteure:%@ plan count:%lu",surface,(unsigned long)drawable.texture.iosurfacePlane);
            }
        } else {
    ...
    if (metalLayerBuffer == NULL) {
        metalLayerBuffer = malloc(size.width*size.height*4);
    }
    
//    [drawable.texture getBytes:metalLayerBuffer bytesPerRow:size.width*4 fromRegion:MTLRegionMake2D(0, 0, size.width, size.height) mipmapLevel:0];
    return drawable;
}
@end
...
-(instancetype)initWithScene:(id<GJImageARScene>) scene captureSize:(CGSize)size
{
    ...
    if (self) {
        ...
        _input = [[GPUImageRawDataInput alloc]initWithBytes:byte size:_captureSize pixelFormat:GPUPixelFormatRGBA type:GPUPixelTypeUByte];
        free(byte);
    }
    return self;
}
-(void)setScene:(id<GJImageARScene>)scene{
    ...
    _scene.updateBlock = ^(){
        if (wkSelf.captureCount % (wkSelf.scene.updateFps / wkSelf.frameRate) == 0) {
            ...
            [wkSelf.input updateDataFromBytes:[(CAMetalLayer*)wkSelf.scene.scene.layer getBuffer] size:wkSelf.captureSize];
            [wkSelf.input processDataForTimestamp:kCMTimeZero];
//            [wkSelf.input updateWithTimestamp:kCMTimeZero];
