...
-(BOOL)startOverlaysWithImages:(NSArray<UIImage *> *)images frame:(CGRect)frame{
    BOOL result = [super startOverlaysWithImages:images frame:frame fps:-1 updateBlock:^void (NSInteger index, GJOverlayAttribute* ioAttr, BOOL *ioFinish) {
    ...
    _imageFrame = _startImageFrame = frame;
    _cameraRotation = (CMRotationRate){0,80/360.0*3.14,80/360.0*3.14};
    if (!_manager) {
...
-(BOOL)startOverlaysWithImages:(NSArray<UIImage *> *)images frame:(CGRect)frame fps:(NSInteger)fps updateBlock:(OverlaysUpdate)update{
	return [self startOverlaysWithImages:images frame:frame];
